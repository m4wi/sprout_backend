...
async function createReservation(greenpointId) {
  ...
  const res = await fetch(`${API_BASE}/greenpoints/${greenpointId}/reservations`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
...
function renderFeed(filter = 'all') {
  ...
  data.forEach(async gp => {
    ...
    avatar.src = user?.avatar_url ? user.avatar_url : 'https://api.dicebear.com/7.x/initials/svg?seed=' + (user?.username || 'user');
    ...
    reserveBtn.addEventListener('click', async () => {
      ...
      reserveBtn.textContent = ok ? 'Reservado' : 'Error';
      if (!ok) reserveBtn.disabled = false;
    });
